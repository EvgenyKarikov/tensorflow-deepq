    ...
    def create_variables(self):
        ...
        with tf.name_scope("taking_action"):
            self.observation = tf.placeholder(
                tf.float32, self.observation_batch_shape(None), name="observation")
            self.action_scores = tf.identity(self.q_network(
                self.observation), name="action_scores")
            tf.summary.histogram("action_scores", self.action_scores)
            #tf.histogram_summary("action_scores", self.action_scores)
            self.predicted_actions = tf.argmax(
                self.action_scores, dimension=1, name="predicted_actions")

        with tf.name_scope("estimating_future_rewards"):
            # FOR PREDICTING TARGET FUTURE REWARDS
            self.next_observation = tf.placeholder(
                tf.float32, self.observation_batch_shape(None), name="next_observation")
            ...
            target_values = tf.reduce_max(self.next_action_scores, reduction_indices=[
                                          1, ]) * self.next_observation_mask
        ...
        self.summarize = tf.summary.merge_all()#tf.merge_all_summaries()
    ...
    def store(self, observation, action, reward, newobservation):
        ...
        if self.number_of_times_store_called % self.store_every_nth == 0:
            self.experience.append(
                (observation, action, reward, newobservation))
    ...
    def training_step(self):
        ...
        if self.number_of_times_train_called % self.train_every_nth == 0:
            ...
            samples = random.sample(
                range(len(self.experience)), self.minibatch_size)
